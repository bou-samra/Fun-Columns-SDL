...
int new_column() {
	...
	for (i = 0; i < 8; i++) {
		for (j = 0; j < 18; j++) {
			if (grid[0][j][i] == 0) { colcnt[i]++;}		// empty space in each column
		}
	}
	for (i = 0; i < 8; i++) {
		if (colcnt[i] > 2) {totcol_avail++;}			// total available columns
	}
	if (totcol_avail == 0) {
		printf("game over\n");} else {				// no more columns available
			randc = randombytes_uniform(totcol_avail) + 0;	// choose random number from available columns
			int avl = -1;
			for (i = 0; randc != avl; i++) {		// translate avail column to actual column
				if (colcnt[i] > 2) {avl++;}
